@echo off
REM *** Batch process LAS files with a PIPELINE - Add header ***
echo(
...
for %%f in (%indir%\*.las) do (
	...
	echo Read %%f
	echo Write !outname!
	pdal pipeline -i C:\Users\Darren\Documents\PDAL\Pipeline_AddHeader.json --readers.las.filename=%%f --writers.las.filename=!outname!
)
set T1=%time%
echo Start Time: %T0%
echo End Time: %T1%