@echo off
REM *** Batch process LAS files with a PIPELINE - Reproject with orthometric height ***
echo(
...
for %%f in (%indir%\*.las) do (
	...
	echo Read %%f
	echo Write !outname!
	pdal pipeline -i C:\Users\Darren\Documents\PDAL\Pipeline_ReprojectOrtho.json --readers.las.filename=%%f --writers.las.filename=!outname!
)
echo(
set T1=%time%
echo Start Time: %T0%
